...
/**
 * Ein Termin erscheint auf der Ebene, auf der er stattfindet.
 *
 * Der Anlass: Seit die Vorwerte der Direktwahlen je Amt eingelesen werden,
 * kennt die App 25 zusätzliche Wahltage – fast alles Bürgermeisterwahlen
 * einzelner Gemeinden. Angezeigt wurden sie auf der **Kreisebene**: Die
 * Kopfzeile von `/hildesheim/` bot sieben Termine an, fünf davon Wahlen einer
 * einzigen Kommune, und die Terminseiten dahinter waren leer.
 *
 * Der Prüfstein in den Fixtures ist die Bürgermeisterwahl der Gemeinde
 * Nordstemmen vom 13.09.2020 (Termin-Id `2020`): Sie gilt für Nordstemmen und
 * für sonst niemanden im Landkreis Hildesheim.
 */
const ohneFolgen = (pfad: string) =>
	fetch(`${BASIS}${pfad}`, { redirect: "manual" });

/**
 * Wartet, bis der 13.09.2020 im Bestand ist.
 *
 * `warteAufDaten` fragt die Kreisebene ab – dort gibt es diesen Wahltag nicht
 * mehr, das ist ja der Punkt. Gewartet wird deshalb auf die Wahlleitung, die
 * ihn führt. Der Archivlauf holt die Termine der Reihe nach; der 13.09.2020
 * kommt nach der Kommunalwahl 2021.
 */
...
test.describe("Termine je Ebene", () => {
	...
	test("die Kopfzeile des Kreises zeigt keine Gemeinde-Wahltage", async ({
		...
		// Die Bürgermeisterwahl Nordstemmen 2020 und die vier weiteren
		// Direktwahl-Vorwerte Hildesheimer Kommunen gehören hier nicht hin.
		await expect(
			termine.getByRole("link", { name: /Nordstemmen/ }),
		).toHaveCount(0);
		await expect(termine.getByRole("link")).toHaveCount(2);

		// Auch die Terminseite des Kreises bleibt bei den zwei kreisweiten.
	...
	test("die Kopfzeile einer Gemeinde zeigt ihre eigenen Wahltage", async ({
		...
		// Und der Verweis bleibt auf dieser Ebene – nicht über die Kreisseite,
		// die diesen Wahltag gar nicht führt.
		...
		// Die Nachbargemeinde hat an dem Tag nicht gewählt: kein Termin in der
		// Kopfzeile, keine Seite.
	...
	test("die Kreis-Terminseite eines Gemeinde-Wahltags führt zur Wahlleitung", async () => {
		// Genau eine Hildesheimer Wahlleitung führt den 13.09.2020, die gemeinte
		// Seite ist also eindeutig – die Adresse war eine Weile im Umlauf und
		// bleibt benutzbar, statt in einer Sackgasse zu enden.
		...
		// Wo mehrere Wahlleitungen denselben Wahltag führen, meint die
		// Kreis-Adresse keine bestimmte Seite: Am 26.05.2019 haben im Landkreis
		// Goslar Bad Harzburg und Seesen gewählt, der Landkreis selbst nicht.
		// Dann 404 wie bei jedem unbekannten Termin – die Seiten der beiden
		// gibt es weiterhin.
		...
		// Und im Landkreis Emsland ist derselbe Tag die Landratswahl, also ein
		// Wahltag des ganzen Kreises – dort bleibt die Terminseite.
	...
	test("die Schnittstelle trennt kreisweite Termine von denen der Wahlleitungen", async ({
		...
		// Wo der 13.09.2020 hingehört, steht bei der Wahlleitung – ein Skript
		// muss es nicht raten.
		...
		// Kreisweit abgefragt: 404 – aber mit dem Weg zur richtigen Ebene.
		...
		// Bei der Wahlleitung selbst geht dieselbe Wahl auf.
		...
		// Und bei der Nachbargemeinde bleibt es beim 404.
		expect(
			(
