/** Der Zeiger auf das zuletzt gesehene Paket – ohne Browser prüfbar. */
...
const paket = (id: number): BeitragAnsicht => ({
	id,
...
describe("der Zeiger", () => {
		...
		const { hole, gefragt } = gegenstelle([
			{
				...
				letzte: 14,
				pakete: [paket(13), paket(14)],
			},
		]);
		const z = beitragZeiger(hole);
		await z.hole(ORT, 12);
		const schub = await z.hole(ORT, 14);
		expect(schub?.pakete.map((p) => p.id)).toEqual([13, 14]);
		...
		const { hole } = gegenstelle([
			{
				topic: "hildesheim/03254026",
				letzte: 99,
				pakete: [paket(5), paket(6)],
			},
		]);
		const z = beitragZeiger(hole);
		await z.hole(ORT, 4);
		const schub = await z.hole(ORT, 99);
		expect(schub?.pakete).toHaveLength(2);
	...
	it("sagt es, wenn nichts übersprungen wurde", async () => {
		const { hole } = gegenstelle([
			{ topic: "hildesheim/03254026", letzte: 6, pakete: [paket(5), paket(6)] },
		]);
		const z = beitragZeiger(hole);
		await z.hole(ORT, 4);
	...
	it("hält zwei gleichzeitige Pings zu einem Abruf zusammen", async () => {
		const { hole, gefragt } = gegenstelle([
			{ topic: "hildesheim/03254026", letzte: 3, pakete: [paket(3)] },
		]);
		const z = beitragZeiger(hole);
		await z.hole(ORT, 2);
...
describe("aufnahmeUrl", () => {
	it("führt über die Paketkennung, nicht über den Satz", () => {
